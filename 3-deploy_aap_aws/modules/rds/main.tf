...
locals {
  ...
  create_gateway_rds_instance = var.infrastructure_gateway_count > 0 ? 1 : 0
}
...
resource "aws_db_instance" "gateway" {
  count = local.create_gateway_rds_instance
  ...
  identifier = "aap-infrastructure-${var.deployment_id}-gateway-db"
  ...
  tags = merge(
    {
      Name = "aap-infrastructure-${var.deployment_id}-gateway-db"
    },
    var.persistent_tags
  )
}
